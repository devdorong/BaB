...
const MatchingListPage = () => {
  ...
  const [sortOption, setSortOption] = useState<'최신순' | '거리순'>('최신순');
        ...
        for (const matching of matchings) {
          ...
          if (restaurant) {
            ...
            processed.push({
              ...matching,
              tags,
              category: restaurant.interests?.name || '기타',
  ...
  useEffect(() => {
    console.log(processedMatchings);
  }, [matchingList]);
  ...
  const paginatedMatchings = sortedMatchings
          ...
          <ul className="flex flex-col gap-x-[30px] gap-y-[24px] list-none p-0 m-0">
            {loading ? (
              ...
              paginatedMatchings.map((item, index) => {
                ...
                const distanceNum =
                  userPos && item.latitude && item.longitude
                    ? getDistanceNum(userPos.lat, userPos.lng, item.latitude, item.longitude)
                    : 0;
              ...
              <p className="text-center text-babgray-500 py-10">해당 카테고리의 매칭이 없습니다.</p>
            )}
          </ul>
        </div>
