...
export const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  useEffect(() => {
    ...
    const { data: subscription } = supabase.auth.onAuthStateChange(async (event, newSession) => {
      console.log('Auth state changed:', event, newSession?.user?.email); // 디버깅용
      setLoading(true);
      ...
      if (event === 'SIGNED_IN' && newSession?.user) {
        ...
        if (!profile) {
          ...
          const ok = await createProfile(newProfile);
          console.log(ok ? '프로필 생성 완료' : '프로필 생성 실패');
        }
      }
      if (event === 'SIGNED_OUT') {
        setSession(null);
  ...
  const value = { session, user, signUp, signIn, signOut, loading };
